...
for iyear in range(initial_year, final_year + 1, 1):
    ...
    filenames_wnd += files
    # fp:
    files = sorted(glob.glob(data_path + "WW3-GLOB-30M_" + "%s" % iyear + "*_fp.nc"))
    filenames_fp += files
...
for iyear in np.unique(years):
    ...
    save_netcdf_deresolved_hs(
        ...
        lat=lat_hs_dr,
        time=time_iyear,
        output="../data/ww3_deresolved_hs_%s" % iyear + ".nc",
        summary=summary,
    )
...
for iyear in np.unique(years):
    ...
    save_netcdf_deresolved_wsp(
        ...
        lat=lat_wnd_dr,
        time=time_iyear,
        output="../data/ww3_deresolved_wsp_%s" % iyear + ".nc",
        summary=summary,
    )
...
for iyear in np.unique(years):
    ...
    # Set summary variable:
    summary = (
        "Data contained in this netCDF file is derived from the wave hindcast product produced by French Research Institute for Exploitation of the Sea (IFREMER) using the WAVE-height, WATer depth and Current Hindcasting III (WW3) wave model forced by Climate Forecast System Reanalysis (CFSR) winds (ftp://ftp.ifremer.fr/ifremer/ww3/HINDCAST). Thus, this data is an intermediate product. Here, WW3 peak frequency (fp) is averaged in 1 degree longitude by 1 degree latitude bins from 66N to 66S over the time period of 1 January %s"
        % iyear
        + " to 31 December %s" % iyear
        + " at daily intervals. Data points are averaged using a running mean. Deresolved fp data is stored in a 3-dimensional (time, latitude, longitude) masked array."
    )
    ...
    save_netcdf_deresolved_fp(
        ...
        lat=lat_fp_dr,
        time=time_iyear,
        output="../data/ww3_deresolved_fp_%s" % iyear + ".nc",
        summary=summary,
    )
