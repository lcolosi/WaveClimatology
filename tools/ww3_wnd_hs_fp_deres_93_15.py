#WW3 Deresolution Hs, WSP, and fp program 
...
from save_netcdf_fields import add_global_atrributes, save_netcdf_deresolved_hs, save_netcdf_deresolved_wsp, save_netcdf_deresolved_fp
...
for iyear in range(initial_year,final_year+1,1):
    ...
    #obtain filenames:
    #Hs: 
    files = sorted(glob.glob('../data/WW3-GLOB-30M_' + '%s' %iyear + '*.nc')) 
    filenames_hs += files
    #Wnd: 
    files = sorted(glob.glob('../data/WW3-GLOB-30M_' + '%s' %iyear + '*.nc'))
    filenames_wnd += files
...
######## fp ########
lon_fp_dr = np.floor(running_mean(lon_fp, k_dim = [2,1], task = 'deresolve'))
lat_fp_dr = np.floor(running_mean(lat_fp, k_dim = [2,1], task = 'deresolve'))
...
for f in filenames_fp:
    ...
    for iday in np.unique(days):
        ...
        #Average temporally
        fp_d = np.ma.mean(fp_h, axis = 0)
        
        #Decrease the resolution of the the wsp matrix via convolution: 
        fp_conv = running_mean(data = fp_d, k_dim = [2,2], task = 'deresolve')
        
        #Extract hs data from -66 to 66 degrees:
        trunc = abs(int((min(lat_fp_dr)-lat_min)/dlat))
        fp_c = fp_conv[trunc:len(lat_fp_dr)-trunc-2,:]  
        
        #Center the data over the Pacific: 
        fp_shift, lon_shift = shift_grid(fp_c, lon_fp_dr, dlon = 180)
        
        #save the average daily array in a 3D array: 
        fp_ww3_cfsr_d[i,:,:] = fp_shift
        time_d_fp.append(itime[0])
...
lat_fp_dr = lat_fp_dr[trunc:len(lat_fp_dr)-trunc-2]
    
#Loop through years to save deresolved Hs, Wsp, and fp data
...
for iyear in np.unique(years):
    ...
    summary = 'Data contained in this netCDF file is derived from the wave hindcast product produced by French Research Institute for Exploitation of the Sea (IFREMER) using the WAVE-height, WATer depth and Current Hindcasting III (WW3) wave model forced by Climate Forecast System Reanalysis (CFSR) winds (ftp://ftp.ifremer.fr/ifremer/ww3/HINDCAST). Thus, this data is an intermediate product. Here, WW3 wind speed (WSP) is averaged in 1 degree longitude by 1 degree latitude bins from 66N to 66S over the time period of 1 January %s' %iyear + ' to 31 December %s' %iyear + ' at daily intervals. Data points are averaged using a running mean. Deresolved WSP data is stored in a 3-dimensional (time, latitude, longitude) masked array.'
    
    #Save ith year binned swh data
    save_netcdf_deresolved_hs(hs = hs_iyear, lon = lon_hs_dr, lat = lat_hs_dr, time = time_iyear, output = '../data/ww3_deresolved_hs_%s' %iyear + '.nc', summary = summary)
...
for iyear in np.unique(years):
    ...
    #Set summary variable: 
    summary = 'Data contained in this netCDF file is derived from the wave hindcast product produced by French Research Institute for Exploitation of the Sea (IFREMER) using the WAVE-height, WATer depth and Current Hindcasting III (WW3) wave model forced by Climate Forecast System Reanalysis (CFSR) winds (ftp://ftp.ifremer.fr/ifremer/ww3/HINDCAST). Thus, this data is an intermediate product. Here, WW3 significant wave height (Hs) is averaged in 1 degree longitude by 1 degree latitude bins from 66N to 66S over the time period of 1 January %s' %iyear + ' to 31 December %s' %iyear + ' at daily intervals. Data points are averaged using a running mean. Deresolved Hs data is stored in a 3-dimensional (time, latitude, longitude) masked array.' 
    
    #Save ith year binned swh data
    save_netcdf_deresolved_wsp(wsp = wsp_iyear, lon = lon_wnd_dr, lat = lat_wnd_dr, time = time_iyear, output = '../data/ww3_deresolved_wsp_%s' %iyear + '.nc', summary = summary)


######## fp ########
#set year time arrays that correspond to the year at which wsp data was collected 
years = np.array([y.year for y in time_d_fp])

#Convert time_d to an array: 
time_array = np.array(time_d_fp)
...
for iyear in np.unique(years):
    ...
    #Call ith year for deresolved wsp and time
    fp_iyear = fp_ww3_cfsr_d[ind_year,:,:]
    time_iyear = time_array[ind_year]
    
    #Set summary variable: 
    summary = 'Data contained in this netCDF file is derived from the wave hindcast product produced by French Research Institute for Exploitation of the Sea (IFREMER) using the WAVE-height, WATer depth and Current Hindcasting III (WW3) wave model forced by Climate Forecast System Reanalysis (CFSR) winds (ftp://ftp.ifremer.fr/ifremer/ww3/HINDCAST). Thus, this data is an intermediate product. Here, WW3 peak frequency (fp) is averaged in 1 degree longitude by 1 degree latitude bins from 66N to 66S over the time period of 1 January %s' %iyear + ' to 31 December %s' %iyear + ' at daily intervals. Data points are averaged using a running mean. Deresolved fp data is stored in a 3-dimensional (time, latitude, longitude) masked array.' 
    
    #Save ith year binned swh data
    save_netcdf_deresolved_fp(fp = fp_iyear, lon = lon_fp_dr, lat = lat_fp_dr, time = time_iyear, output = '../data/ww3_deresolved_fp_%s' %iyear + '.nc', summary = summary)