...
for iyear in range(initial_year,final_year+1,1):
    ...
    #obtain filenames: 
    files_fp = sorted(glob.glob(data_path + 'fp/WW3-GLOB-30M_' + '%s' %iyear + '*_fp.nc'))
    files_wnd = sorted(glob.glob(data_path + 'Wnd/WW3-GLOB-30M_' + '%s' %iyear + '*_wnd.nc'))
...
save_netcdf_prob_swell(monthly_prob_swell=prob_swell_m, seasonal_prob_swell=prob_swell_s, lon=lon_wnd, lat=lat_wnd, monthly_time=np.arange(1,13,1), seasonal_time=np.arange(1,5,1), output=output, summary=summary)
