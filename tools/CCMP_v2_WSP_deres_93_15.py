...
for iyear in range(initial_year,final_year+1,1):
    ...
    files = sorted(glob.glob('../data/CCMP_Wind_Analysis_' + '%s' %iyear + '*_V02.0_L3.0_RSS.nc'))
...
for iyear in np.unique(years):
    ...
    summary = 'Data contained in this netCDF file is derived from the Cross Calibrated Multi-Platform version 2 (CCMP2) wind vector analysis produced by Remote Sensing Systems product (available at www.remss.com). Thus, this data is an intermediate product. Here, CCMP2 wind speed (WSP) is averaged in 1 degree longitude by 1 degree latitude bins from 66N to 66S over the time period of 1 January %s' %iyear + ' to 31 December %s' %iyear + ' at daily intervals. Data points are averaged using a running mean. Deresolved WSP data is stored in a 3-dimensional (time, latitude, longitude) masked array.'
    
    
    #Save ith year binned swh data
    save_netcdf_deresolved_wsp(wsp = wsp_iyear, lon = lon_dr, lat = lat_dr, time = time_iyear, output = '../data/CCMP2_deresolved_wsp_%s' %iyear + '.nc', summary = summary)
