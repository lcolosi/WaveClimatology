...
#Import functions 
from save_netcdf_fields import add_global_atrributes, save_netcdf_binned_swh, save_netcdf_binned_swh_wsp
...
for iyear in range(initial_year,final_year+1,1):
    ...
    files = sorted(glob.glob('../data/wm_%s' %iyear + '*.nc'))
...
for iyear in np.unique(years):
    ...
    summary = 'Data contained in this netCDF file is derived from the French Research Institute for Exploitation of the Sea (IFREMER) cross-calibrated along-track satellite altimetry significant wave height (SWH) product (ftp://ftp.ifremer.fr/ifremer/cersat/products/swath/altimeters/waves). Thus, this data is an intermediate product. Here, IFREMER corrected along-track SWH (swhcor) is placed in 1 degree longitude by 1 degree latitude bins from 80N to 80S over the time period of 1 January %s' %iyear + ' to 31 December %s' %iyear + ' at daily intervals. Data points in bins are averaged and number of observations averaged in bins is recorded. Binned SWH data is stored in a 3-dimensional (time, latitude, longitude) masked array.'
    
    #Save ith year binned swh data
    save_netcdf_binned_swh(swh = swh_iyear, nobs = nobs_iyear, lon = grid_lon, lat = grid_lat, time = time_iyear, output = '../data/IFREMER_binned_alt_swh_%s' %iyear + '.nc', summary = summary)
    
    
#Save SWH and WSP into one large file:

#Set summary variable: 
summary = 'Data contained in this netCDF file is derived from the French Research Institute for Exploitation of the Sea (IFREMER) cross-calibrated along-track satellite altimetry significant wave height (SWH) and wind speed (WSP) product (ftp://ftp.ifremer.fr/ifremer/cersat/products/swath/altimeters/waves). Thus, this data is an intermediate product. Here, IFREMER corrected along-track SWH (swhcor) and WSP are placed in 1 degree longitude by 1 degree latitude bins from 80N to 80S over the time period of 1 January 1993 to 31 December 2015 at daily intervals. Data points in bins are averaged and number of observations averaged in bins is recorded. Binned SWH and WSP data are stored in a 3-dimensional (time, latitude, longitude) masked arrays.'

#Save data
save_netcdf_binned_swh_wsp(swhcor_array, wspcor_array, N_array, grid_lon, grid_lat, time_array, output = '../data/IFREMER_binned_alt_swh_wsp_1993_2015.nc', summary = summary)
